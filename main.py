...
class Graph:
   ...
   to print the path '''
    ...
    def genEdges(self,debug):
        ...
        pool = Pool(nodes=int(sys.argv[1]))
...
#for i in range(20):
    ...
    colors = cm.rainbow(np.linspace(0, 1, odd_node_count/2))
...
np.save('sphere_filled_print_path_Final.npy',flat_print_path)

if plot_results:
    import matplotlib.pyplot as plt
    list_length = [len(part) for part in flat_print_path]
    dist = OrderedDict(sorted(Counter(list_length).items()))
    plt.bar(range(len(dist)), list(dist.values()), align='center',log=True)
