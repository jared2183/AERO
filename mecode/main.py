...
class G(object):
    ...
    def view(self, backend='matplotlib', outfile=None, color_on=False,nozzle_cam=False,export=False):
        ...
        if backend == 'matplotlib':
            ...
            #Scene setup
            scene_angle = 0
            x = np.sin(np.radians(scene_angle%360))
            z = -np.cos(np.radians(scene_angle%360))
            vp.scene.forward = vp.vec(x,-1,z) 
            vp.scene.forward = vp.vec(-1,-1,-1)
            vp.scene.width = 1080*1.0
            vp.scene.height = 1080*1.0
                ...
                def __init__(self, nozzle_diameter, nozzle_length, start_location=vp.vec(0,0,0), start_orientation=vp.vec(0,1,0)):
                    ...
                    if self.compound_mode:
                       ...
                       #Create interior surface
                        ...
                        # Create simpler nozzle out of fundamental parts #####
                        self.nozzle_components = []
                        self.nozzle_components_offsets = [1.2,1.2,1.2+4.5+2.6,1.2+4.5+2.6,10.0,10.0]
                        self.nozzle_components.append(vp.cone(pos=start_location+vp.vec(0,nozzle_length+1.2,0),axis=vp.vec(0,-1.6,0),radius=1.6,color=vp.color.blue,opacity=1.0,shininess=0.1))
                        self.nozzle_components.append(vp.cylinder(pos=start_location+vp.vec(0,nozzle_length+1.2,0),axis=vp.vec(0,4.5,0),radius=1.6,color=vp.color.blue,opacity=1.0,shininess=0.1))
                        self.nozzle_components.append(vp.cone(pos=start_location+vp.vec(0,nozzle_length+1.2+4.5+2.6,0),axis=vp.vec(0,-5.0,0),radius=2.7,color=vp.color.blue,opacity=1.0,shininess=0.1))
                        self.nozzle_components.append(vp.cylinder(pos=start_location+vp.vec(0,nozzle_length+1.2+4.5+2.6,0),axis=vp.vec(0,9.0,0),radius=2.7,color=vp.color.blue,opacity=1.0,shininess=0.1))
                        self.nozzle_components.append(vp.box(pos=start_location+vp.vec(0,nozzle_length+10.0,0),length=6.0,height=13.5,width=0.8,color=vp.color.blue,opacity=1.0,shininess=0.1))
                        self.nozzle_components.append(vp.cylinder(pos=start_location+vp.vec(0,nozzle_length+12.0,0),axis=vp.vec(0,15.0,0),radius=5.25,color=vp.color.gray(0.8),opacity=1.0,shininess=0.1))
                ...
                def abs_move(self, endpoint, feed=2.0,print_line=True,tail_color = None):
                    ...
                    for inter_move in np.transpose(inter_points): 
                        ...
                        if self.compound_mode:
                            ...
                            self.head.pos.x = inter_move[0]
                            self.head.pos.z = inter_move[2]

                            for component in self.nozzle_components:
                                component.pos.x = inter_move[0]
                                component.pos.z = inter_move[2]

                            self.head.pos.y = inter_move[1]
                            for component, offset in zip(self.nozzle_components,self.nozzle_components_offsets):
                                component.pos.y = inter_move[1]+self.nozzle_length+offset
                        ...
                        if not self.compound_mode:
                            ...
                            if export:
                                ...
                                #self.scene_angle -= 2/2.5
                                #x = np.sin(np.radians(self.scene_angle%360))
                                #z = -np.cos(np.radians(self.scene_angle%360))
                                #vp.scene.forward = vp.vec(x,-1,z) 
                            self.pov_count += 1
                            #
                            #if self.pov_count % 100:
                                #np.savetxt('render_time_{}.csv'.format(self.pov_count),np.array(self.render_time),delimiter=',')
                            #self.render_time.append(time.time()-self.start_time)
            ...
            def run():
                ...
                for count, (pos, color) in enumerate(zip(position_hist[1:],self.color_history[1:]),1):
                    print("{}/{}".format(count,len(position_hist[1:])))
                    X, Y, Z = pos
